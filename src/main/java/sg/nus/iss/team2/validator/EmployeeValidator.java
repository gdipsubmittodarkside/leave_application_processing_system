    ...
    public void validate(Object target, Errors errors){
        ...
        if (!emp.getStatus().equalsIgnoreCase("resigned") &&
            !emp.getStatus().equalsIgnoreCase("existing")){
                errors.rejectValue("status", "error.Employee.status", "Status should be \'existing\' or \'resigned\'");
            }
        ...
        if (m.find()){
            errors.rejectValue("name", "error.Employee.name", "Name should not contain any digits or special characters");
        }
    }
}
